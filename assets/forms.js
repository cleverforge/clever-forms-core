(function () {
	...
	function getValue(form, key) {
		...
		if (elements[0].type === 'radio') {
			var checkedRadio = Array.from(elements).find(function (element) { return element.checked; });
			return checkedRadio ? checkedRadio.value : '';
		}
		if (elements[0].type === 'checkbox') {
			return Array.from(elements).filter(function (element) { return element.checked; }).map(function (element) { return element.value; }).join(',');
		}
	...
	function initializeForm(wrap) {
		...
		function showPage() {
			...
			window.requestAnimationFrame(function () {
				signaturePads.forEach(function (item) { item.pad.resize(); });
			});
		}
		...
		function applyConditions() {
			form.querySelectorAll('[data-condition-field]').forEach(function (box) {
				...
				var show = true;
				if (operator === 'equals') { show = actual === wanted; }
				else if (operator === 'not_equals') { show = actual !== wanted; }
				else if (operator === 'contains') { show = actual.indexOf(wanted) !== -1; }
				else if (operator === 'not_empty') { show = actual !== ''; }
				box.classList.toggle('is-hidden', !show);
				box.querySelectorAll('[required]').forEach(function (element) {
		...
		wrap.querySelectorAll('.clever-signature').forEach(function (box) {
		...
		function persistDraft() {
			...
			signaturePads.forEach(function (item) {
				if (!item.pad.isEmpty()) {
		...
		form.querySelectorAll('.clever-signature-data').forEach(function (hidden) {
			if (!hidden.value) { return; }
			var box = hidden.closest('.clever-signature');
			var item = signaturePads.find(function (candidate) { return candidate.hidden === hidden; });
			if (box && item) {
		...
		function copyValues() {
			form.querySelectorAll('[data-copy-from]').forEach(function (box) {
		...
		form.querySelectorAll('input[type=checkbox][data-max-selections]').forEach(function (checkbox) {
			checkbox.addEventListener('change', function () {
				...
				if (group.filter(function (item) { return item.checked; }).length > max) {
		...
		form.querySelectorAll('textarea[data-min-words], textarea[data-max-words]').forEach(function (textarea) {
			...
			function updateWordCount() {
				...
				if (note) { note.textContent = words.length + ' word' + (words.length === 1 ? '' : 's'); }
				var message = '';
				if (min && words.length < min) { message = 'Enter at least ' + min + ' words.'; }
				if (max && words.length > max) { message = 'Use no more than ' + max + ' words.'; }
		...
		form.querySelectorAll('.clever-slider').forEach(function (slider) {
			var output = slider.parentElement.querySelector('output');
			function updateSlider() { if (output) { output.textContent = slider.value; } }
			slider.addEventListener('input', updateSlider);
			updateSlider();
		});

		form.querySelectorAll('.clever-list').forEach(function (list) {
			...
			list.addEventListener('click', function (event) {
				if (!(event.target instanceof Element) || !event.target.classList.contains('clever-list-remove')) { return; }
		...
		function updatePrice() {
			...
			var subtotal = products.reduce(function (total, product) { return total + (parseFloat(product.dataset.productPrice || '0') || 0); }, 0);
			form.querySelectorAll('.clever-calc-subtotal').forEach(function (field) { field.value = subtotal.toFixed(2); });
			form.querySelectorAll('[data-money-type=subtotal] strong').forEach(function (output) { output.textContent = subtotal.toFixed(2); });
		}
		form.addEventListener('change', updatePrice);

		if (wrap.dataset.preview === '1') {
			form.addEventListener('submit', function (event) {
				if (form.dataset.previewed === '1') { return; }
				...
				form.querySelectorAll('.clever-field[data-field-key]').forEach(function (field) {
					...
					var values = Array.from(controls).filter(function (control) {
						return control.type !== 'hidden' && control.type !== 'file' && ((control.type !== 'checkbox' && control.type !== 'radio') || control.checked);
					}).map(function (control) { return control.value; }).filter(Boolean);
			...
			for (var i = 0; i < signaturePads.length; i += 1) {
				...
				if (item.hidden.dataset.required === '1' && !item.hidden.value) {
					...
					if (field) {
						...
						if (targetIndex >= 0) { pageIndex = targetIndex; showPage(); }
					}
					if (item.status) { item.status.textContent = 'Signature is required.'; }
					item.canvas.focus();
					return;
				}
			}
